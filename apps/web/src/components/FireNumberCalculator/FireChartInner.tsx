...
export default function FireChartInner({ data, fireNumber, currency, locale }: Props) {
  const fmt = (v: number) =>
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      <LineChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
        ...
        <Legend />
        <ReferenceLine y={fireNumber} stroke="var(--color-accent)" strokeDasharray="4 4" label={{ value: 'FIRE target', position: 'insideTopRight', fontSize: 11 }} />
        <Line type="monotone" dataKey="balance" stroke="var(--color-ink)" dot={false} name="Portfolio" />
      </LineChart>
    </ResponsiveContainer>
  );
}
